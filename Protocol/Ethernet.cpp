...
{
    auto *ethernet_hdr = (struct ethernet_header *)packet_content;
    int i;
    int flag = 0;
    for (i = 0; i < 6; i++)
    {
    ...
    {
        //printf("crc = %u,data = %u\n",crc,*((u_int32_t *)(packet_content + len - 4)));
...
void ethernet_protocol_packet_callback(u_char *argument, const struct pcap_pkthdr *packet_header, const u_char *packet_content)
{
    ...
    switch (ethernet_type)
    {
        case 0x0800:
            ...
            is_exit = (bool)network_ipv4_recv(upper_buffer);
            break;
        case 0x0806:
            printf("Upper layer protocol: ARP\n");
            //printf("enter recv arp\n");
    ...
    //printf("argument = %s\n",argument);
    if(is_exit) pcap_breakloop((pcap_t*) argument);
...
int ethernet_send_packet(u_int8_t *upper_buffer, u_int8_t *destination_mac, u_int16_t ethernet_type)
{
    ...
    //printf("size_of_package = %d",size_of_packet);
